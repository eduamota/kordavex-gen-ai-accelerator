...
**Interaction Mode**: Delivery | Diagnostic (Diagnostic when failures occur) (see common/socratic-interaction.md)
...
## Step 9: Push to Git Repository

**After all tests pass**, push all commits to the remote repository:

1. Verify all unit commits are present: `git log --oneline`
2. Push to remote:
   ```bash
   git push -u origin main
   ```
3. Verify push succeeded
4. Log push status in audit.md with timestamp and remote URL
5. If push fails (e.g., repo doesn't exist on GitHub yet), inform the user:
   - Provide the command to create the repo: `gh repo create doitintl/<accelerator-name> --private`
   - Retry push after repo creation
